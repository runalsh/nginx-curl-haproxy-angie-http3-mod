...
RUN apk add --no-cache build-base wget ca-certificates openssl gnupg unzip make zlib-dev pkgconfig libtool cmake automake autoconf build-base linux-headers openssl-dev pcre-dev wget zlib-dev ca-certificates uwsgi uwsgi-python3 supervisor git
...
RUN ls -la /tmp/build/nginx/nginx-${NGINX_VERSION}
...
RUN cd /tmp/build/nginx/nginx-${NGINX_VERSION} && \
	...
	--add-module=/tmp/build/module/ngx_http_proxy_connect_module --add-module=/tmp/build/module/nginx_upstream_check_module --add-module=/tmp/build/module/ngx_cache_purge --add-module=/tmp/build/module/ngx_http_substitutions_filter_module --add-module=/tmp/build/module/echo-nginx-module
