...
RUN cd /tmp/build/nginx && \
    wget https://nginx.org/packages/mainline/debian/pool/nginx/n/nginx/nginx_${NGINX_VERSION}-1~bookworm.debian.tar.xz && \
    tar -xJf nginx_${NGINX_VERSION}-1~bookworm.debian.tar.xz -C ./nginx-${NGINX_VERSION}
...
RUN cd /tmp/build/nginx/nginx-${NGINX_VERSION} && \
    ...
    dpkg -i /tmp/build/nginx/nginx_${NGINX_VERSION}-1~bookworm_amd64.deb
