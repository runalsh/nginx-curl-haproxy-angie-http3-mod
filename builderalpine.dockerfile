...
RUN apk add --no-cache build-base wget curl ca-certificates gnupg unzip make zlib-dev pkgconfig libtool cmake automake autoconf linux-headers pcre-dev uwsgi uwsgi-python3 supervisor samurai libunwind-dev perl-dev libstdc++ libssl3 libcrypto3 openssl openssl-dev git luajit-dev libxslt-dev brotli-dev nghttp2-dev pcre2 shadow go gcc libc-dev readline-dev tar g++ geoip-dev libressl pcre2-dev libpsl-dev libidn2-dev
